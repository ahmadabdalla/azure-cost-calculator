...
This is the authoritative mapping of Azure services → category folders. Each entry shows the exact API `serviceName` (case-sensitive), the category folder it belongs to, and common aliases users may search for. When a contributor creates a new service reference file, they MUST place it in the category listed here.
