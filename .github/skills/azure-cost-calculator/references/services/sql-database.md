...
- **Default storage**: Both General Purpose and Business Critical default to 32 GB max data size. Storage is always billed separately from compute at the tier’s per-GB rate — there is no "free included" data storage in the vCore model. You are charged for the configured maximum data size, not actual usage. Backup storage equal to the configured max data size is provided at no extra charge.
...
>
> **Trap (RI skuName)**: For Reservation pricing, the `skuName` is `'vCore'` (NOT `'8 vCore'`). If you include `-SkuName '8 vCore'` on an RI query, the API returns **zero results**. Only use `-SkuName` for PAYG queries. For RI queries, omit `-SkuName` and multiply the per-vCore price by your vCore count manually.
>
