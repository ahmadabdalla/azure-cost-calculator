...
| Upgrading Waza: the version is pinned in a handful of files; a partial update breaks CI    | Update all of these together: `.github/actions/install-waza/action.yml` (version + checksum), `.devcontainer/Dockerfile` (curl URL), `docs/ops/evals.md` (version row + schema links in References), `docs/ops/dev-container.md` (table row). YAML files (`.waza.yaml`, `eval.yaml`, `eval-mock.yaml`, all task files) intentionally do not carry `$schema` URLs: `waza check` validates against the bundled schema in whichever binary is installed, so those files need no touch on version bumps |
