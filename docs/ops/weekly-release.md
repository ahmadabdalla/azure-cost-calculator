...
| `create-release-pr` step fails with `gh` error | Invalid `gh` CLI flags or API failure in `create-release-pr.sh` | Check step logs for `::error::` annotations; `gh pr list` failure surfaces as "Failed to query open PRs from GitHub API", `gh pr create` failure as "Failed to create release PR" |
