...
1. **Check the routing map** — open `references/service-routing.md` and confirm your service is listed. If it is not there, open an issue first.
2. **Check existing files** — search `references/services/` to make sure the file does not already exist.
...
```powershell
.\Explore-AzurePricing.ps1 -ServiceName 'Your Service Name'
# Or search broadly:
.\Explore-AzurePricing.ps1 -SearchTerm 'keyword' -Top 50
```
...
Copy `references/services/TEMPLATE.md` into the correct category folder:

```bash
cp references/services/TEMPLATE.md references/services/{category}/{filename}.md
```

Use the filename suggested in `service-routing.md`. Use lowercase with hyphens (e.g., `app-service.md`).
...
- `category`: the folder name this file lives in — must match one of the 18 categories in `shared.md`
...
```powershell
.\Validate-ServiceReference.ps1 -Path references/services/{category}/{filename}.md -CheckAliasUniqueness
```
...
The API value is case-sensitive. Use `Explore-AzurePricing.ps1` to find the exact value.
...
- Do **not** annotate headers with "(case-sensitive)" — this is a universal rule in `shared.md`
