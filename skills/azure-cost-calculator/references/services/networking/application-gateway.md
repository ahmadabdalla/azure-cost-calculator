...
- **AGIC (Application Gateway Ingress Controller)**: Free AKS add-on with no separate billing meter. All costs flow through the linked Application Gateway v2 resource (Standard v2 or WAF v2 only). Estimate AGIC costs using the standard App Gateway v2 query pattern above; do not add any extra line items for AGIC itself.
