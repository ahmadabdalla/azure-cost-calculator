...
> **Warning**: **Empty-region pricing**: scripts require a Region filter and return nothing for Private DNS. Use `Region: Zone 1` as a workaround, or query the API directly with `armRegionName eq ''`, appending `&currencyCode=` for the user's currency.
