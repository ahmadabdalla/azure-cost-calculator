...
```
Per order = service_retailPrice + shipping_retailPrice + max(0, daysOnSite - includedDays) x extraDay_retailPrice
Disk total = (service_retailPrice + shipping_retailPrice) + daily_retailPrice x diskCount x max(0, daysOnSite - 3)
Multi-device = Per order x deviceCount
```

## Notes

- Included days before overage: Disk = 3, Data Box 100 TB = 10, Heavy = 20
