...
| `Standard Jobs Light Compute DBU`            | `Standard Jobs Light Compute`            | `1 Hour`      | Light jobs (Standard)             |
| `Premium SQL Analytics DBU`                  | `Premium SQL Analytics`                  | `1 Hour`      | Classic SQL warehouse (Premium)   |
| `Standard SQL Analytics DBU`                 | `Standard SQL Analytics`                 | `1 Hour`      | Classic SQL warehouse (Standard)  |
...
| `Premium Serverless Realtime Inferencing DBU`| `Premium Serverless Realtime Inferencing`| `1 Hour`      | Model serving (+ per-launch fee)  |
| `Launch Charge Serverless Realtime Inferencing DBU` | `Launch Charge Serverless Realtime Inferencing` | `1` | Per-launch event fee (not hourly) |
...
| `Premium Clean Rooms Collaborator Collaborator` | `Premium Clean Rooms Collaborator`    | `1/Day`       | Per collaborator per day (×30)    |
...
Clean Rooms        = collaborator_retailPrice × 30 × collaboratorCount   (billed per day)
```
...
- **Photon variants**: Photon-accelerated SKUs (e.g., `Premium Jobs Compute Photon`) have the same DBU rate as non-Photon but process data faster, reducing total DBU-hours consumed
- **Delta Live Tables**: Separate DLT meters at Core, Pro, and Advanced levels (e.g., `Premium Pro Compute Delta Live Tables`); Photon DLT variants also exist at same rates
...
- **14-day free trial**: Free Trial and POC Non-Billable SKUs return zero cost; ignore these for cost estimation
...
- **Global-only meters**: `Premium Serverless GPU Compute Capacity Block` and `Premium Anthropic Serverless Inference` are available only in Global region under `Azure Databricks`
