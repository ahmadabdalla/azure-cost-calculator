...
> **Trap (multi-node clusters)**: Each cluster has multiple node roles (Head ×2, Worker ×N, Zookeeper ×3). Estimate each role separately — query the per-node hourly rate, multiply by node count and 730.
...
| `S30 Disk`         | `S30`              | `HDInsight Storage`            | `1/Month`     | Standard HDD for Kafka   |
| `P30 Disk`         | `P30`              | `HDInsight Storage`            | `1/Month`     | Premium SSD for Kafka    |
...
Kafka Disks     = disk_retailPrice × diskCount              (Kafka clusters only)
ESP Surcharge   = esp_retailPrice × totalCores × 730        (ESP-enabled only)
Total Monthly   = Cluster Monthly + Kafka Disks + ESP Surcharge + Azure Storage
```
...
- **Cluster types**: Hadoop, Spark, HBase, Kafka, Interactive Query, ML Services — each requires different node topologies (Head ×2, Worker ×1+, Zookeeper ×3)
- **Kafka disks**: Kafka clusters require managed disks (S30 Standard or P30 Premium) under `HDInsight Storage`; other cluster types use external Azure Storage (Blob/ADLS)
