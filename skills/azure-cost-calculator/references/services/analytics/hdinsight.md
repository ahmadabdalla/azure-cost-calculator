...
> **Trap (inflated totals)**: An unfiltered `ServiceName 'HDInsight'` query returns 134+ meters across 20 products spanning all VM series. The `totalMonthlyCost` sums every node size and is meaningless. Always filter by `ProductName` for the specific VM series.
...
### Add-on software (empty-region meters)

API: https://prices.azure.com/api/retail/prices?$filter=serviceName eq 'HDInsight' and productName eq 'HDInsight Add-on Software' and armRegionName eq ''&currencyCode={currencyCode}
...
### General-purpose v5 node: D4ads v5

ServiceName: HDInsight
ProductName: HDInsight Dadsv5 Series
SkuName: D4ads v5
MeterName: D4ads v5
...
- **Add-on surcharges**: Product `HDInsight Add-on Software` (empty-region meters; use the `API:` template above): ESP surcharge per-core/hour, R Server surcharge per-core/hour, Kafka surcharge is zero-priced; ESP also auto-provisions an ID Broker node
...
- **VM series**: 20 products span A, Av2, D (Da v4 + legacy Dv2), Dadsv5, ESv3, Eadsv5, Eav4/Easv4, Edsv5, Edv5, Ev3, F, FS, FSv2, G, Lasv3, NC; filter by `ProductName` for specific family
