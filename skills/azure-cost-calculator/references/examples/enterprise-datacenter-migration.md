...
- 6× Azure Virtual Machines, Standard_D4s_v5 (Linux), 1-Year Reserved — customer self-service portal
  - 1× Premium SSD v2, 128 GB per VM
...
- 6× Azure Virtual Machines, Standard_F16s_v2 (Windows), 1-Year Reserved, AHUB enabled — CAD batch rendering servers
  - 1× Premium SSD v2, 512 GB per VM
...
- 1× Azure SQL Managed Instance, General Purpose tier, Gen5, 8 vCores, Zone Redundant, 1-Year Reserved, SQL AHUB enabled — LOB application databases (CRM, HR, procurement)
  - 512 GB storage
- 1× Azure Database for PostgreSQL, Flexible Server, General Purpose, Standard_D4ds_v5 (4 vCores, 16 GB RAM), Zone Redundant HA — Linux application databases
  - 512 GB storage
...
- Replica managed disks provisioned automatically by Site Recovery in West Europe (billed as standard managed disks in the DR region)

## Backup & Retention

- Azure Backup, 100 Azure VMs protected, LRS redundancy
  - 90 instances ≤ 500 GB each (standard protected instance tier)
  - 10 instances 500 GB–2 TB each (large protected instance tier)
  - 15 TB total backup storage (LRS)
  - Retention: 30 daily, 12 monthly, 1 yearly restore points
- Azure SQL Managed Instance automated backups (built-in, included in SQL MI pricing)
...
- 1× Log Analytics workspace, Pay-as-you-go, 50 GB/day ingestion, 90-day interactive retention — centralized log aggregation for all VMs, PaaS diagnostics, and security logs
- 1× Application Insights (workspace-based), 10 GB/month ingestion, 90-day retention — customer portal and external web applications
- 1× Application Insights (workspace-based), 5 GB/month ingestion, 90-day retention — corporate intranet and internal applications

## Security

- Microsoft Defender for Cloud, Servers Plan 2 — 70 Windows Server VMs
- 1× Microsoft Sentinel (SIEM), Pay-as-you-go, 30 GB/day ingestion — security event monitoring, threat detection across all infrastructure, compliance audit logging
...
- Currency: EUR
- Commitment: Mixed — 3-Year Reserved (10 VMs), 1-Year Reserved (62 VMs), Pay-As-You-Go (28 VMs); 1-Year Reserved for SQL MI
