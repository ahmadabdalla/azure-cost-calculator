...
These services return pricing in **USD only** — either because they are API-unavailable or because they are listed under the `Global` region. **Services not listed here support native non-USD currencies** — pass the target `currencyCode` directly to the script and the API returns prices in that currency without any FX conversion.
